...
const createMainWindow = (port) => {
  ...
  const executeOnWindow = (command, callback) => {
    return mainWindow.webContents.executeJavaScript(command).then(callback).catch(console.error);
  };
...
const installExtensions = async () => {
  ...
  const extensions = ["REACT_DEVELOPER_TOOLS"].map((extension) =>
    installer.default(installer[extension], isForceDownload)
  );
